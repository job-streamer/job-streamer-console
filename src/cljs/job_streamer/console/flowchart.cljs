...
(defn- save-job-control-bus [xml svg]
...
(defn render []
  (let [modeler js/window.bpmnjs ;; From global scope.
    ...
    (if job-name
      (api/request (str "/" app-name "/job/" job-name)
                   {:handler (fn [{:keys [job/bpmn-xml-notation]}]
                               (.importXML modeler bpmn-xml-notation
                                           import-xml-cb))})
      (.importXML modeler initial-diagram
                  import-xml-cb))
